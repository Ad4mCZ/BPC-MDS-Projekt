...
class NewFolderHandler(FileSystemEventHandler):
    ...
    def process_file(self, path):
        if path.endswith('.m3u8') and path not in self.detected_names:
            self.detected_names.add(path)
            self.detected_name = os.path.splitext(os.path.basename(path))[0]
            print(f"Nový nebo upravený .m3u8 soubor detekován: {self.detected_name}")
...
#            self.observer.stop()
    ...
    def on_modified(self, event):
        """
        Reaguje na úpravu souboru.
        """
        self.process_file(event.src_path)

    def on_any_event(self, event):
        """
        Reaguje na přejmenování souboru.
        """
        self.process_file(event.src_path)
